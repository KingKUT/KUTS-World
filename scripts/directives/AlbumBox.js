	...
	.directive('albumBox', function (SecurityFactory, $compile) {
		return {
			...
			link: function (scope, element, attrs, ctrl) {
				...
				/*
				function init() {
					var html = '<div class="album-wrapper"><a class="buttons prev" href="#" ng-click="prevStackItem($index, album.media.length)"><</a><a class="buttons next" href="#" ng-click="nextStackItem($index, album.media.length)">></a>' +
						'<div class="stack" ng-repeat="file in album.media">' +
						'<div class="numbertext">{{currentIndex + 1}} / {{album.media.length}}</div>' +
						'<re-file re-file-obj="file" re-index=$index lf-unknow-class="strUnknowIconCls" security/></div></div>';
					element.replaceWith($compile(html)(scope));
				}
				init();*/

				//scope[attrs.reAlbums] = scope.reAlbums;
			}
		};
	});
	...
	.directive('reAlbum', function (SecurityFactory) {
		return {
			...
			link: function (scope, element, attrs) {
				if (scope.reAlbumObj) {
					console.log(scope.reAlbumObj);
				}
				scope.currentIndex = 0;
				/*scope.$watchCollection('reAlbumObj', function (newVal) {
